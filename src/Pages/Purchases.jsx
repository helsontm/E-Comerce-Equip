...
const Purchases = () => {
    ...
    useEffect(() => {
        ...
        axios
            ...
            .get("https://ecommerce-app-ktdy.onrender.com/purchases", getConfig())
            .then(resp => setPurchases(resp.data))
            .catch(error => console.error(error))
            .finally(() =>   dispatch(setIsLoading(false)))
    }, [])
    //Funcion cambio de formato de fhecha
    const changeDate=(date)=>{
        let  changed= new Date(date)
        let mounth = changed.getUTCMonth() + 1; // se agrega 1 porque empieza con 0
    let day= changed.getUTCDate();
    let year = changed.getUTCFullYear();
//retorna nuevo formato de fhecha
